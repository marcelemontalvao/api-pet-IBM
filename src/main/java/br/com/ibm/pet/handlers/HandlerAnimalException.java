...
public class HandlerAnimalException {

    @ExceptionHandler(value = ResponseStatusException.class)
    protected ResponseEntity<MensagemErro> handlerAnimalJaCadastrado(ResponseStatusException exception) {
        if (exception.getStatus() == HttpStatus.OK){
            return ResponseEntity.status(HttpStatus.OK).body(new MensagemErro("Animal já existente."));
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body(new MensagemErro("Animal não encontrado."));
    }
